...
The host quickstart must be running: `mise run mcp:http` and `mise run a2a` from `agents/python/`, `mise run gateway:host` from the repository root, and Ollama serving `qwen3:4b` for the A2A scenario. On Kubernetes, port-forward agentgateway and the raw services first and override the `*_URL` environment variables.

For the fake-model comparison, stop Ollama so port `11434` is free, run `mise run model:fake`, and restart the A2A process with `AGENT_GATEWAY_ENABLED=true`. The existing host and k3d gateway profiles already route model calls to that host port, so the A2A script and every other layer stay identical. The fake deliberately refuses streaming; keep `AGENT_A2A_STREAMING=false` so the experiment changes only inference.
