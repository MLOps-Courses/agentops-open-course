...
- Model calls: native Gemini uses the SDK's ``HttpRetryOptions``; the gateway
  path passes ``timeout``/``max_retries`` to the OpenAI SDK (see ``model.py``).
...
class ToolDeadlineError(TimeoutError):
    """A tool call exceeded ``AGENT_TOOL_TIMEOUT_S`` and was cancelled."""
...
def with_resilience(func: Callable[..., dict[str, Any]]) -> Callable[..., Any]:
    ...
    The sync function runs in a worker thread so ``asyncio.wait_for`` can
    actually enforce the deadline (ADK calls sync tools inline on the event
    loop, where a timeout could never fire). ``functools.wraps`` preserves the
    signature and docstring ADK reads to build the tool schema.
    """
