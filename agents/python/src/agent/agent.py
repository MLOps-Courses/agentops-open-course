...
platform against a bundled offline dataset. Model requests use native Gemini locally or
the governed agentgateway route. The implementation composes tools, skills, MCP, runbook
retrieval, workflows, and A2A delegation.
"""
...
def _instruction() -> str:
    ...
    ``AGENT_PROMPT_URI`` (e.g. ``prompts:/ops-copilot-instruction/2``) pins the
    instruction to a version registered in the self-hosted MLflow registry
    (Ch. 7.0) — so behavior is attributable to an exact prompt version. Unset,
    the committed text below is used and no MLflow server is required.
    """
