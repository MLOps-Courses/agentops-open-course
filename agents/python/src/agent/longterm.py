...
disposable state directory (never the seed dataset), are isolated per user, and
pass PII redaction before persisting — memory is a data store like any other.
``mise run data:reset`` clears it along with the rest of the runtime state.
"""
