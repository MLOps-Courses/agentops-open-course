...
Ports are stable across the host and cluster profiles, so one map covers every `port-forward` and probe you run in this chapter and the next. Reach a service only through the route in the last column — most are cluster-internal and surface locally only via a temporary forward:
...
| `8080`        | agentops-agent | A2A (diagnosis only)           | direct; normally reached via gateway |
...
| `9093`        | Alertmanager   | alert routing (host profile)   | host Compose                         |
| `3002`        | Grafana        | dashboards (host profile)      | host Compose                         |

The host profile ([Chapter 7](../7. Observability/index.md)) also uses gateway readiness `:15021`. Every published listener binds to loopback or a ClusterIP; no port here is exposed by an Ingress or LoadBalancer.
