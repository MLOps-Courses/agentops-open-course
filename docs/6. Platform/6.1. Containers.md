...
That the process ran the whole time under `--user 10001:10001` (never root) and `--read-only` is the checkpoint made concrete: `/app/data` is immutable, and only the mounted `/app/state` volume and the `/tmp` tmpfs are writable — a healthy `/healthz` under those flags is the proof.
