...
The multi-stage Dockerfile pins the uv and Python base images by version and digest. The build stage uses the locked project without development dependencies; the final stage copies only the virtual environment and seed:
...
Expected: user `app`/UID 10001 (runtime representation can vary), `8080/tcp`, and the Python module entrypoint.
