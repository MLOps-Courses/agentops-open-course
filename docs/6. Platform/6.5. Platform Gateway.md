...
The smoke first echoes a synthetic function result through `:4000`. It then requires the A2A agent to reach a completed task and return `INC-002`'s stable ID, severity, and title. An exact one-call gateway-counter delta proves `get_incident` ran once. Both turns are read-only, and the proof remains valid if the incident's runtime status changed earlier.
