...
The result is a directed allow-list: solid edges below are the only flows any pod may open, and everything else is refused by `default-deny-egress`. Note that even the agent cannot reach the raw MCP server directly — it must go through the gateway.
...
\* The model upstream is the per-overlay exception on the gateway pod only: local Ollama `:11434`, or GKE Vertex over HTTPS plus WIF `:987/:988`. Every pod also keeps a namespace-wide DNS allowance to `kube-system`.
