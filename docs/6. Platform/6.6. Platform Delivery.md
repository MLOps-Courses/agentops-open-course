...
Never `cp` a live SQLite file: a copy taken mid-transaction can be torn. Safe hot copies go through SQLite itself, which holds the page-level locks for you. Both the backup and the restore enforce one shared contract — a snapshot only becomes restorable once it is complete and published, so an interrupted attempt can never masquerade as the newest good one:
