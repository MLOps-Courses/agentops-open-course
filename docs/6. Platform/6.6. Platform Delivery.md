...
- Artifact Registry with seven-day cleanup for untagged images.
...
1. `runtime.db` — ADK sessions and A2A tasks from the persistent server (Ch. 4.2).
1. `incidents.db` — the writable dataset copy, including approved `restart_service`/`resolve_incident` mutations and the append-only `audit_log` (Ch. 3.2).
...
On the host, [backup-state.sh](https://github.com/MLOps-Courses/agentops-open-course/blob/main/infra/scripts/backup-state.sh) snapshots every state database with `VACUUM INTO`, verifies each copy with `PRAGMA integrity_check`, and keeps the newest seven snapshots under the gitignored `.state-backups/`:

```bash
./infra/scripts/backup-state.sh
```

In Kubernetes, the `agentops-state-backup` CronJob in [state-backup.yaml](https://github.com/MLOps-Courses/agentops-open-course/blob/main/infra/k8s/base/state-backup.yaml) applies the same idea nightly with the Python stdlib backup API — it reuses the course image, so no new tool or image pin enters the supply chain — writing timestamped snapshots from the `agentops-agent-state` PVC into a second `agentops-state-backups` PVC:
...
The drill seeds throwaway state, approves a mock action (the same `UPDATE` plus `audit_log` append the agent commits in one transaction), snapshots it, deletes the state directory, restores it with [restore-state.sh](https://github.com/MLOps-Courses/agentops-open-course/blob/main/infra/scripts/restore-state.sh), and asserts the row counts and the last audit entry survived. Expected final line: `drill passed: 1 audit row(s) and 1 session row(s) survived backup + restore`.

To restore real host state, stop every writer first (the A2A server, the MCP server, any `adk run`/`adk web`), then point the script at a snapshot; it verifies integrity and prints the restored `audit_log` row count:

```bash
./infra/scripts/restore-state.sh .state-backups/<snapshot>
```
...
Copy the newest snapshot back with a short-lived Job that mounts both PVCs and reuses the course image — a plain copy is safe here precisely because nothing is writing:
        ...
        - name: restore
          ...
          args:
            - |
              ...
              snapshot = sorted(p for p in pathlib.Path("/backups").iterdir() if p.is_dir())[-1]
...
Finally, redeploy (`skaffold run -p local` restores the Agent and the MCP replica) and verify the audit trail through the agent: ask for recent audit entries and compare against the pre-drill count. The stopped-writer requirement is the whole reason to practice this before an incident, not during one.
