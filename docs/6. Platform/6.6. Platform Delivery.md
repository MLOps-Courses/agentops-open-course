...
It builds `agentops-agent` from `agents/` and `agentops-mlflow` from `infra/mlflow`, tags them with the abbreviated Git commit, pushes to `SKAFFOLD_DEFAULT_REPO`, renders the chosen Kustomize overlay, and applies it with kubectl. `skaffold dev` then watches your sources and repeats the loop on every change:
