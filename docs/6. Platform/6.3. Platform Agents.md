...
1. `volumeMounts`/`volumes` — the [state PVC](../0.%20Overview/0.7.%20Glossary.md#pvc) and a small writable `/tmp`.
...
1. The overlay patches env values. The main `local-gemini` overlay selects `gemini-3.5-flash` by the named `AGENT_MODEL` field; the optional `local` overlay selects `qwen3:4b-instruct`. GKE keeps its qualified Gemini pin. Model backend is a data-plane change (Chapter 6.5).
...
- read-only root filesystem, a writable 1 Gi [RWO](../0.%20Overview/0.7.%20Glossary.md#rwo) state PVC at `/app/state`, and a 128 MiB `/tmp` `emptyDir`.
...
Kubernetes calls an HTTP endpoint inside a container on a schedule. That [probe](../0.%20Overview/0.7.%20Glossary.md#probe) is run by the **kubelet** — the Kubernetes agent on each node.
...
Expected: one ready agent pod, bound `agentops-agent-state`, and [ClusterIP](../0.%20Overview/0.7.%20Glossary.md#clusterip) `agentops-agent` on 8080.
