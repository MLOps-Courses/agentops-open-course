...
1. The overlay patches env values. The local overlay rewrites `env[0]` (`AGENT_MODEL`) to `qwen3:4b-instruct`; the GKE overlay leaves `gemini-3.6-flash`. Model backend is a data-plane change (Chapter 6.5).
...
The local overlay uses `qwen3:4b-instruct` while the optional GKE overlay uses `gemini-3.6-flash` behind that same gateway endpoint.
