...
The rendered docs gate checks the presence of that metadata without inventing popularity, outcomes, ratings, credentials, or marketing claims. A public release still needs the online publication gate below to prove the deployed canonical URL, sitemap, metadata, and 404 behavior.
...
A green workflow proves the source builds from a clean checkout. By itself it does not prove that the repository is anonymously readable, that the Pages setting is correct, that DNS resolves, that TLS is valid, or that every deployed link works. Those five are what the publication gate below checks.
...
??? note "Deeper: the publication gate, for whoever hosts the site"
    ...
    CLEAN_HOME="$(mktemp -d)"
    # Proves anonymous Git access: no system config, no prompt, no credential helper.
    ...
    curl -fsS \
      ...
      >/dev/null
    # Proves every link in the built site resolves, this time over the network.
    ...
    This is the online counterpart to the offline `mise run check:links`: run it only after `mise run check:docs` has generated `site/`, and note the deep-URL form depends on `use_directory_urls: false`. The exact LinkedIn profile remains a manual browser check because LinkedIn rejects automated clients; every other extracted link stays machine-checked. If any command fails, the verified surface remains the local preview and the release is not publication-complete. The [8.2. Releases](./8.2. Releases.md) gate requires this same anonymous check for any publication release.
...
Follow all changed pages in the local preview, and confirm critical snippets still render from source. Inspect the generated `site/CNAME`. For a publication release, run the anonymous clone/site/source-link gate above from outside any maintainer-authenticated browser or session, and walk the current freshness-audit issue if one is open. Do not advertise or deploy manually from an unvalidated working tree.
