...
Inside `agents/python/src/agent/` there is no package hierarchy to navigate — twenty-three flat modules, each owning exactly one concern, and each docstring naming the chapter that teaches it. That is the repository's "flat over deep, one owner per concern" principle made concrete: to find where a behavior lives you read a filename, not a tree.
