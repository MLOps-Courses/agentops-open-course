...
Within the gateway-backed platform, the application uses one OpenAI-compatible chat-completions interface. Part I uses native Gemini; changing that transport requires behavior requalification. Moving between Ollama, the gateway, and a hosted model changes only `OPENAI_BASE_URL`.
