...
| Interoperability           | An A2A discovery/request demonstration using the reference handoff                                    |
...
The remaining sections describe the larger reference-domain port and platform capstone. Platform learners may start directly from the completed reference after [4.8. Developer Handoff](../4.%20Quality/4.8.%20Developer%20Handoff.md).
...
## What completed derivative should the maintainer commission?

The smallest useful worked capstone is a separate `agentops-developer-support-reference` repository. It would replace the incident seed with sanitized repository, CI, and runbook fixtures; expose read-only failure and ownership tools; retain one approval-gated rerun action; and preserve the course's model, policy, gateway, evidence, and cleanup contracts.

The maintainer should create it only after approving that scope and its long-term ownership. This course must not link an empty repository or imply external evidence exists before that decision.

The handoff contract for the commissioned repository is concrete:

1. A README maps each changed domain surface back to the corresponding capstone milestone and names explicit non-goals.
1. A `course:evidence` manifest from the exact reviewed commit verifies the deterministic gates and locked inputs.
1. One sanitized model trajectory, one gateway smoke result, one approval/audit round-trip, and one local-platform observation are reported separately from deterministic proof.
1. Known model failures, absent production controls, teardown, and the repository's support boundary remain visible.

That proposal is the deliverable here. Creating or publishing the external repository remains a maintainer action.
