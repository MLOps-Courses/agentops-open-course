---
description: Transform the completed Ops Copilot reference into your own open-source agent platform and prove every boundary with reproducible evidence.
---
...
Create a domain-specific agent platform that preserves the course's open contracts while replacing the fictional Ops Copilot domain with a problem you understand. Examples include a developer-support agent, data-quality investigator, security triage assistant, or internal platform runbook copilot.
...
| Model path       | Why is local `qwen3:4b` sufficient for the required path, and which evaluation would justify another model? |
...
Commit-sized milestones are easier to review and debug than one final rewrite. Keep each milestone green before starting the next.
...
| Local model     | `mise run doctor:model` plus one recorded read-only trajectory on `qwen3:4b`                                           |
