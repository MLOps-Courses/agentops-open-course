...
For issue-based freshness evidence, the validator ignores tasks inside code fences and comments, matches the checked task inventory to the exact template in the qualified commit, and records its count and SHA-256. A shortened or expanded issue cannot stand in for that release's review.
