...
Also render both Kubernetes overlays, inspect dependency/image locks, run live evaluations on explicitly recorded models when behavior changed, and review release notes for OSS/cloud/cost/security overclaims.
...
1. Scans each image with Trivy before any push and fails on HIGH/CRITICAL findings, respecting the documented `.trivyignore` entries.
...
For the current repository, verify `CHANGELOG.md` contains only `Unreleased` and `0.1.0` and do not create a tag. A future release requires explicit maintainer intent and the full gate above.
