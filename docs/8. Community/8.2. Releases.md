...
- The two container images (`agent` and `mlflow`) published for that version in GHCR, GitHub's container registry.

Run `git tag --list 'v*' --sort=v:refname | tail -1` to see the highest `v`-prefixed Git tag available in your clone. This page never names a specific version, because a version printed in prose is a version that can rot.
...
A release is worth publishing only when this whole gate is green from a clean checkout:
...
1. **Close a freshness audit from the last 120 days with every checklist box checked** or give the protected release-environment reviewer an explicit one-release waiver reason.
1. **A publication release** additionally requires the anonymous repository/site/source-link gate from [8.4. Documentation](./8.4.%20Documentation.md).
...
1. Dispatch `Release` with that full commit SHA, `v`-prefixed version, and either `issue:<number>` for a recently closed, fully checked freshness audit or `waiver:<reviewed reason>` for this release only; the protected workflow creates the annotated tag and publishes the curated changelog section with its evidence.
...
The workflow then does eight things:
...
1. Builds exact archives for both images with source, revision, version, creation time, license, title, description, and documentation labels.
1. Loads, runtime-smokes, and scans both images, then generates their SPDX SBOMs before any job can write to GHCR; vulnerabilities/secrets fail at HIGH/CRITICAL, and licenses fail outside `trivy.yaml`.
...
1. Promotes both source digests into annotated version indexes only after both publication jobs succeed; failure reconciliation rediscovers current registry state and removes only version indexes that still prove exact ownership.
1. Seals each public version-index digest with a new keyless signature, the same exact SBOM, and public SLSA provenance.
...
Those steps form one fail-closed chain. Each protected write/OIDC job receives authority only after qualification and both read-only preflights.
...
flowchart LR
    ...
    Preflight --> Publish["release environment<br/>push source digest + sign + attest"]
    Publish --> Promote["promote source digests<br/>to annotated version indexes"]
    Promote --> Seal["seal public indexes<br/>sign + same SBOM + provenance"]
    Seal --> Verify["verify digest, tag, labels,<br/>signature, SBOM, provenance"]
    Verify --> Release["annotated tag + complete<br/>published GitHub release"]
```

**Diagram in words:** A maintainer dispatches one full `main` SHA and freshness handoff. Five exact-SHA workflows plus a recent closed audit or one-release waiver qualify it before both images are built, smoked, scanned, and inventoried without registry write access. Protected jobs publish and sign each source digest, promote it into an annotated version index, and seal the public index with the same SBOM and new provenance. A separate read-only job verifies the evidence before a protected job creates the annotated tag and public release.
...
!!! warning "Dependabot does not see every pin in this repository"
    ...
    It covers packages and Dockerfile base images. It does **not** watch the `[tools]` pins in `mise.toml` and `mise.lock`, the Helm chart versions in `infra/helmfile.yaml`, the image digests inside `infra/k8s/**`, or the Wolfi `apk` pins in both runtime Dockerfiles.
...
Dependabot sees only the package half of most of these rows; every other half is hand-maintained, which is exactly why the checklist exists. After merging an upgrade that changes an image, dispatch and complete the next protected release so its GHCR artifacts, signatures, SBOMs, and annotated source tag match the repository again (see the release workflow above).
...
One command, offline and done in seconds:

```bash
mise run check:release-metadata "$(git tag --list 'v*' --sort=v:refname | tail -1)"
```

That passes the highest `v`-prefixed Git tag straight into the check, so nothing here depends on which version you happen to be reading. It verifies that the latest dated entry in `CHANGELOG.md` matches that tag and that `pyproject.toml`, `agents/python/pyproject.toml`, and `CITATION.cff` all carry the same version. This offline metadata check does not claim that GitHub has published the release or its images.
...
- You can say what the published release adds beyond its source commit: scanned images, SBOMs, signatures, provenance, and verification evidence; its tag identifies that exact source.
