...
1. **Close a freshness audit from the last 120 days** or write an explicit waiver for the protected release-environment reviewer.
...
1. Dispatch `Release` with that full commit SHA, `v`-prefixed version, and either `issue:<number>` for a recently closed freshness audit or `waiver:<reviewed reason>`; the protected workflow creates the annotated tag and publishes the curated changelog section with its evidence.
...
1. Requires the requested SHA to equal the current protected `main` head, requires successful CI, Docs, Scan, Eval, and Platform runs for that exact SHA, and records a freshness issue closed within 120 days or an explicit waiver.
...
1. Promotes both recorded digests to the version only after both publication jobs succeed; a failed second promotion reconciles the first version tag.
