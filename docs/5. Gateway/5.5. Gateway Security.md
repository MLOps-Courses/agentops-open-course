...
`infra/scripts/gateway-tls.sh` generates a local CA and a CA-signed server certificate for `localhost`/`127.0.0.1` (30-day validity, gitignored). Clients trust `ca-cert.pem`; the secured profile terminates TLS on every listener:
