...
The result is 24 test files and roughly 239 test functions (more cases after parametrization); the gate fails under 95% branch coverage and, because the deterministic seams are all it touches, runs with no model, no key, and no network.
