...
- Read tools are narrow; gateway mode sends them through an allowlisted MCP route.
...
`.env` is ignored and loaded locally by mise. GKE uses Workload Identity Federation for Vertex and GCS rather than service-account keys. Gitleaks scans staged changes/history, and examples use markers rather than real tokens.
...
The root task runs gitleaks over Git history and Trivy over vulnerabilities, secrets, and misconfiguration. Python `mise run check:vuln` runs pip-audit. CI repeats the same repository task. Any suppression must name one finding, explain reachability/risk, and be revisited when a fix ships.
