...
Retries fix a _flaky_ dependency; they are exactly wrong for a _dead_ one. If a gateway or MCP server is hard down, every read still pays the full `AGENT_MAX_RETRIES` budget — deadline, backoff, deadline — before failing, so a broken dependency turns each turn into a pile of slow, doomed calls. The complementary lever is a **circuit breaker**: after a run of failures it _opens_ and fails fast, then after a cooldown lets a single trial call test recovery.
...
It is **opt-in** — `AGENT_CIRCUIT_BREAKER_ENABLED` defaults to `false`, so the shipped behavior stays retry-only until a builder measures a persistently-failing dependency and turns it on. When enabled, a read that fails `AGENT_CIRCUIT_FAILURE_THRESHOLD` times in a row opens the breaker; further calls raise `CircuitOpenError` immediately instead of paying the retry budget, until `AGENT_CIRCUIT_RESET_TIMEOUT_S` elapses and one trial call is allowed through. Each opening increments the `agentops.circuit.opened_total` counter, so a breaker that flaps is visible in [7.2. Monitoring](../7.%20Observability/7.2.%20Monitoring.md). Like every resilience seam, the breaker only ever wraps idempotent reads — never a write.
...
## How do you freeze every write instantly during an incident?

Human confirmation gates each write one decision at a time; sometimes you need to stop _all_ of them at once — a compromised session, a misbehaving prompt proposing bad remediation, a change window you must not touch — without waiting for a redeploy. `AGENT_WRITES_DISABLED` is that kill-switch. When set, both guarded actions refuse _before_ the approval flow even runs:
...
The refusal is deliberate in three ways. It short-circuits before any state read, so a frozen agent touches nothing. It returns the same actionable `error` shape every other refusal uses, so the model reports "writes are frozen" instead of failing opaquely. And reads keep working — triage, runbooks, and log search still answer, so the on-call engineer can keep investigating while writes are frozen. `test_kill_switch_freezes_writes_before_approval` asserts an approved restart _and_ resolve both refuse with the audit-log row count unchanged. Because it is a single environment variable, it is a runtime lever an incident responder can pull in seconds — the [7.7. Incident Response](../7.%20Observability/7.7.%20Incident%20Response.md#what-can-you-change-to-mitigate-without-a-redeploy) page lists it first among the mitigations that need no redeploy.
