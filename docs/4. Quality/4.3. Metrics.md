...
[4.7. Evaluation Reference](./4.7.%20Evaluation%20Reference.md#which-evaluation-task-should-you-run-and-when) owns that policy and maps local hooks, CI, and scheduled runs. Use its classification when filling the scorecard below; this page owns the measurements themselves.
...
A **scorer** is a function that grades one recorded conversation. [4.7. Evaluation Reference](./4.7.%20Evaluation%20Reference.md#what-does-the-mlflow-evaluation-add) owns how the five scorer means are enforced and where that command runs.
...
1. `agentops.circuit.opened_total` — a counter incremented when a tool circuit transitions open, in [`circuit.py`](https://github.com/MLOps-Courses/agentops-open-course/blob/main/agents/python/src/agent/circuit.py).
...
| ----------------- | ---------------------- | -------------------------------- | ---------------------------- | ------------------------- |
| Combined coverage | `mise run test`        | 96%                              | gate (`>= 95%`)              | pass                      |
| Tool trajectory   | `mise run eval`        | 5/15; four critical passes       | minimum 80% + critical cases | fail                      |
...
1. `mise run test` prints the combined line-and-branch coverage total and fails under 95%.
...
- `mise run test` reports combined line-and-branch coverage at or above 95%.
- `mise run eval` clears the wrapper's `80%` aggregate floor and its individual memory-recall, skill-loading, restart-confirmation, and resolution-confirmation gates.
