...
Five tasks cover different layers. Only one is a merge gate; the rest are evidence you run deliberately. Reach for the cheapest one that answers your question:
...
flowchart TD
    ...
    Ollama --> Runs["eval · eval:report · eval:mlflow<br/>no judge"]
...
1. It runs `mise run eval`, `mise run eval:report`, and `mise run eval:mlflow` against the fixed seed data, without the optional judge, and uploads both model-backed logs, the SQLite tracking store, and the MLflow run artifacts as a downloadable workflow artifact.
