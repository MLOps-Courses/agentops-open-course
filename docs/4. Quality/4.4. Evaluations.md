...
Two layers decide the result: ADK grades each case strictly, then the repository wrapper enforces the aggregate floor and three named critical cases.
...
`mise run eval` executes ADK through `evals/run_adk_eval.py`. The wrapper preserves ADK's output, runs every case unless an ADK subprocess fails or produces no valid summary, and reports every named critical miss. It exits non-zero when a critical case misses or fewer than `25%` pass. On a 15-case set, whole-case arithmetic means at least four cases must pass.

The required course path is `qwen3:4b-instruct` on your own machine. The `25%` aggregate floor requires at least four of the fifteen strict cases, so it catches a **collapse** without pretending this 4B model should pass all fifteen. Three named contracts must also pass regardless of that average: full remediation-skill loading, guarded restart confirmation, and guarded resolution confirmation.

The distinction matters: `1.0` is the per-case ADK trajectory threshold in JSON; `0.25` is the wrapper's aggregate case-pass floor in the task. The repeated `--required-case` arguments make critical trajectories non-compensating: ten easy passes cannot hide one missed approval boundary. Neither wrapper setting is a `tool_trajectory_avg_score` threshold.
...
A 4B run finishes green with four or more strict case passes and all three named critical cases. The exact result varies, but the summary and wrapper verdict have this shape:
...
Required strict ADK cases passed: remediation-loads-skill, resolve-needs-approval, restart-needs-approval.
```
...
Once a baseline exists, four rules apply:
...
- `mise run eval` reports that the aggregate floor and all three named critical cases passed; the saved eval-history JSON identifies every strict case that missed.
