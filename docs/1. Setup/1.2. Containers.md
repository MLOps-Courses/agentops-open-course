...
`agents/python/Dockerfile` uses a multi-stage build and runs as UID/GID `10001` on a Python 3.13 Distroless runtime. The final image contains the locked application environment plus the immutable `agents/data` seed, but no shell or package manager. It starts `python -m agent.server` and listens for A2A on port `8080`.
