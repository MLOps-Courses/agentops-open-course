...
The wrapper closes that gap with a small asyncio loopback relay ([`loopback-relay.py`](https://github.com/MLOps-Courses/agentops-open-course/blob/main/infra/scripts/loopback-relay.py), controlled by `AGENTOPS_GATEWAY_LOOPBACK_RELAY=auto`). On native Linux it binds the wrapper-owned bridge gateway and forwards MCP, A2A, and model traffic to host loopback. On Docker Desktop it is skipped. Metrics stay inside that bridge: Compose Prometheus scrapes the gateway container directly.
