...
See [`scripts/doctor.sh`](https://github.com/MLOps-Courses/agentops-open-course/blob/main/scripts/doctor.sh). Because this block only prints, `doctor:platform` passes whether or not the `local` cluster exists — that is exactly how the "validate without creating a cluster" promise is implemented. It fails only on a missing pinned tool, a missing project virtualenv, a non-executable gateway wrapper (`infra/scripts/gateway-host.sh`), an unreachable Docker engine (either `docker info` or `docker compose version`), or a missing `helm-diff` plugin.

A green run on a machine that has installed the tools but not yet created the cluster prints exactly five lines — the success side of each probe above:
...
The `cluster` line is the tell that the "validate without creating a cluster" promise held: the check reported your context state and stopped. If you have no `.env`, the second line instead reads `env        optional .env is absent`; once you reach Chapter 6 and start the cluster, the last line becomes `cluster    k3d-local selected`.
