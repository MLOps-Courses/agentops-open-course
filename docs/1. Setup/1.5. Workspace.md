...
flowchart LR
    ...
    state -->|"mise run data:reset = rm -rf .state"| seed
```
...
Only configuration and model-backed tasks load that `.env`: `config:check`, `run`, `web`, `a2a`, and the `eval*` tasks declare `env = { _.file = { path = "../../.env" } }`, while `install`, `check`, `test`, and `format` inherit nothing from it. That split is a frequent surprise — setting a variable in `.env` and expecting `mise run test` to observe it will not work, because the offline gate is deliberately dotenv-free so its result never depends on your local secrets.
