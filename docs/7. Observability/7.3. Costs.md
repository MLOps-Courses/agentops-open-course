...
## What does a filled-in estimate look like?

The formula stays abstract until you put numbers in it, so here is one worked example — the rate is **illustrative**, not a vendor quote. Suppose one session's trace reports `agentops.tokens.session.input = 1500` and `agentops.tokens.session.output = 400`, and an operator has set `AGENT_INPUT_PRICE_PER_1K=0.20` and `AGENT_OUTPUT_PRICE_PER_1K=0.60`:
...
That `0.54` is what `estimate_cost` would write to `agentops.cost.session.estimate` for that session. On the default local path both prices are `0.0`, so the same session estimates `0.0` — the mechanism runs, but the number is only meaningful once an operator sets published rates and reconciles the total against the provider's own invoice. Treat it as a budgeting signal, never as billing truth.
