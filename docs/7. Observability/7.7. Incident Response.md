...
1. **Freeze every write.** If the agent is proposing or making bad state changes — a compromised session, a prompt gone wrong, a change window you must not touch — set `AGENT_WRITES_DISABLED=true`. Both guarded actions refuse before approval while reads keep working, so triage continues while nothing can mutate ([4.5](../4.%20Quality/4.5.%20Guardrails.md#how-do-you-freeze-every-write-instantly-during-an-incident)). It is the fastest blast-radius cut you have.
...
1. **Narrow the surface.** Disable an opt-in that is misbehaving — turn `AGENT_SEMANTIC_RETRIEVAL` back off to fall back to the deterministic keyword scorer, or route reads back off a failing MCP gateway. If a read dependency is hard down, `AGENT_CIRCUIT_BREAKER_ENABLED=true` stops the agent from paying the full retry budget on every doomed call ([4.5](../4.%20Quality/4.5.%20Guardrails.md#how-does-the-agent-shed-load-when-a-dependency-stays-down)); if the model endpoint is down and you have a validated smaller model, `AGENT_MODEL_FALLBACK` fails over to it ([5.4](../5.%20Gateway/5.4.%20Model%20Gateway.md#how-does-the-agent-fail-over-when-the-primary-model-is-down)).
