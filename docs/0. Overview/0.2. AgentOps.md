...
| **Actions have consequences**: a tool call can change state          | Mutating tools pause for a human yes (`require_confirmation=True`). On approval they write the state change plus an audit row whose schema's triggers reject row updates and deletes, in one SQLite transaction — `actions.py` | [4.5. Guardrails](../4. Quality/4.5. Guardrails.md)                                                  |
