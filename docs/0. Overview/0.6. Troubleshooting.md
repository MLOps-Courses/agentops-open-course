...
Symptom: a variable you set in the root `.env` is ignored by the offline gates, so a config change seems to have no effect.

This is deliberate: only the configuration and model-backed tasks load the dotenv — `config:check`, `run`, `web`, `a2a`, and the `eval*` tasks. The offline gates (`test`, `check`, `redteam`, `mcp`) load no dotenv at all, so a passing suite proves the committed defaults hold rather than whatever is in your local `.env`. Confirm the agent's resolved configuration with `mise run config:check` (from `agents/python`), which is one of the tasks that does read `.env`. To exercise a variable inside the offline suite, set it in the test itself or pass it inline for that one command.
