...
| Optional Ollama calls refuse to connect         | `mise run doctor:model`                                                                            | [reach Ollama or the gateway](#why-cant-the-agent-reach-ollama-or-the-model-gateway)  |
...
| Coverage below 95% after an edit                | `cd agents/python && uv run pytest --cov=agent --cov=evals --cov-branch --cov-report=term-missing` | [coverage gate](#why-does-mise-run-test-fail-the-coverage-gate-after-my-edit)         |
...
The provider configuration and model-construction boundary enforce these rules:

1. Remove the retired `AGENT_GATEWAY_ENABLED` variable: select native `gemini` or the explicit `openai-compatible` gateway/local transport.
1. `AGENT_MODEL_PROVIDER=openai-compatible` requires an OpenAI-compatible base URL and a non-empty **client marker** — the `OPENAI_API_KEY` value, which local Ollama accepts as any non-secret string. Use `http://127.0.0.1:11434/v1` with optional direct Ollama or `http://127.0.0.1:4000/v1` with agentgateway in Chapter 5.
...
This is deliberate. The **dotenv** — the root `.env` file — is loaded only by configuration and model-backed tasks. The non-model tasks (`test`, `check`, `redteam`, `mcp`) load no dotenv, so a passing suite proves the committed defaults hold rather than whatever is in your local `.env`. These agent-level gates are offline. Root `check:vuln`, used by maintainers and CI, is the separate package-advisory query.
...
## Why does Gemini reject my first request?

Validate configuration before retrying a failed hosted request.

Run `cd agents/python && mise run config:check`. A missing key fails locally without sending a prompt; read-only tools and offline exercise checks remain usable without credentials. A provider `401` or `403` requires checking key access and supported regions. A `429` means quota or rate limits: wait according to the provider response, shorten the exercise, or continue with offline checks. Avoid repeated retries or enabling billing without reviewing the provider terms.

See [1.4. Providers](../1.%20Setup/1.4.%20Providers.md) for configuration. Never paste the key or complete request bodies into an issue.
...
This is a common failure on the optional CPU-only Ollama path. The shipped deadline is 60 seconds with two retries (`model_timeout_s` and `max_retries` in `agents/python/src/agent/config.py`), sized for a warm GPU. A local Qwen3-4B on CPU takes seconds to minutes per turn — a multi-tool investigation makes several such calls, and the first one also pays the model load.
...
The offline suite enforces at least 95% **combined line-and-branch coverage**. Branch measurement is enabled, but the threshold applies to the combined total. New source lines need tests that exercise them, including error branches.

Run `cd agents/python && uv run pytest --cov=agent --cov=evals --cov-branch --cov-report=term-missing` to collect coverage and show the uncovered lines, then add a deterministic test for each. Never lower the threshold or add `# pragma: no cover` to force green.
