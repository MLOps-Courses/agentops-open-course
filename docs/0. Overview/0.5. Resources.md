...
!!! abstract "In one glance"
    ...
    - **Time:** about 11 minutes end to end, but this is a lookup page — skim it, then come back.

## Which sources should you trust first?

This page is a trust map, not a link dump: it ranks where to look and says what each source is authoritative _for_. The one place that decides what this course actually supports is the repository itself.

There is a hierarchy, and which half of it applies depends on what you are asking:

- For "what does _this_ course do," the installed source, the **lockfile** (which records exact dependency versions), and the passing tests in this repository outrank any prose, including these docs.
- For "how does the underlying tool work in general, and what is its full API," the upstream primary documentation outranks a blog post or a model's recollection.

Agent APIs move fast and most of these projects are pre-1.0, so a tutorial written six months ago can describe a schema that no longer exists.

Reach for the upstream docs when you are extending the reference beyond what the course shows — a tool option, a policy field, a **CRD** attribute (a field on a Kubernetes custom resource) the pages never needed:
...
The map below follows the same split: the repository first for this course, the upstream primary docs first for the tool in general. Orientation material sits below both and never overrules either:
...
The highest-authority resource for this course is not external at all. The repository's stated contract is that _docs mirror source_ and _the source remains the verification surface_, so when a page and the code diverge, the code wins.

After the clone in [1.0. System](../1.%20Setup/1.0.%20System.md), run `ls` at the repository root once so the paths below have a shape. Read these in-repo artifacts as first-class references, in rough order of authority:

- **`agents/python/tests/` — the real behavioral contract.** The offline test suite is what actually pins the agent's behavior; it runs with no model or network and enforces at least 95% combined line-and-branch coverage. If you want to know what an invariant _guarantees_ rather than what the prose _claims_, read the test that asserts it.
...
- **`CHANGELOG.md`.** Release-facing changes in Keep a Changelog form (entries grouped by change type) — the fastest way to see what moved between versions before you trust an older example.
...
??? note "Deeper: which editors and assistants do people use with this course?"
...
For an academic or technical citation, use [`CITATION.cff`](https://github.com/MLOps-Courses/agentops-open-course/blob/main/CITATION.cff) at the repository root, and cite the version you actually used.

??? note "Deeper: what does `CITATION.cff` record?"

    It records the released version, the author, the repository and site URLs, and both the `MIT` and `CC-BY-4.0` grants; on GitHub, the **Cite this repository** menu exports the current metadata directly. Cite the version you actually used — the reference is a moving target, so read the version out of the file rather than out of this page.
...
One external companion exists: the [MLOps Coding Course](https://mlops-coding-course.fmind.dev/). It covers the broader software and model-lifecycle foundations this course assumes — packaging, testing, CI, and deployment for machine-learning systems — without the agent-specific stack. Reach for it when a gap is really about general MLOps engineering rather than agents. For anything agent-specific, the in-repo artifacts above are the companion material.

## Where do the key terms live?

Any term the course teaches as a concept is defined in one line in [0.7. Glossary](./0.7.%20Glossary.md). The list is alphabetical and covers the vocabulary each chapter introduces — agent, tool, MCP, A2A, RAG, HITL, guardrail, trace, k3d, Kustomize overlay, WIF, and the rest. Each entry links back to the section that introduces it, so start there when a term is unfamiliar.
...
??? note "Deeper: where do the pins live?"
...
Come back to this page when two sources disagree, not to read it end to end. It settles one question: which source wins.

**You are done when:**

- You can name the authority for "what does _this_ course do": the repository — its tests, its locked dependencies, its code.
- You can name the authority for "how does this tool work in general": that project's primary documentation, linked in the table above.
- You know where the pinned versions live, so you can tell a stale example from a current one.
- You know where a report goes: a course issue, an upstream tracker, or the private process in `SECURITY.md`.

Continue to [0.6. Troubleshooting](./0.6.%20Troubleshooting.md) when you know which of those two authorities to open first.
