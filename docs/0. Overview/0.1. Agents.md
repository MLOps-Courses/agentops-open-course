...
A useful working definition: an agent is a **model** that runs in a **loop**, calling **tools** and reading their results, until the task is done. The Ops Copilot you build in this course is an agent in exactly this sense — given "investigate incident 42", it decides to look up the incident, read the relevant runbook, check the service status, and propose a fix.
