...
1. **Policy** — the validation, redaction, budget, and error handling wrapped around the loop. ADK callbacks: validation and error handling from `guardrails.py`, PII redaction from `pii.py`, and the token budget from `budget.py` — all attached to the agent at the runtime boundary rather than buried in the prompt ([4.5. Guardrails](../4. Quality/4.5. Guardrails.md)).
