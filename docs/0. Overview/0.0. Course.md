...
Each chapter admits exactly the tools it needs, and a matching `doctor` profile checks that tier before you start. The ladder is additive: a passing higher tier implies the lower ones. Run the profile for the tier you are entering, not everything at once.
