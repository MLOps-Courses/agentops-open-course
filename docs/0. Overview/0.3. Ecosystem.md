...
| Local model           | [Ollama](https://github.com/ollama/ollama) and [Qwen3](https://huggingface.co/Qwen/Qwen3-4B) | A model-backed path without a hosted provider account.              |
...
The agent uses the same endpoint contract locally and on GKE. The gateway changes its backend from Ollama/Qwen3 to Vertex AI/Gemini and obtains cloud identity through Workload Identity Federation; the Python agent does not acquire a provider-specific key.
