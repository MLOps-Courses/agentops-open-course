...
Because the model's advertised maximum and the serving window are two different numbers. `ollama show qwen3:4b-instruct` prints the architecture's context length. [Ollama chooses the default serving window](https://docs.ollama.com/context-length) from available VRAM:
