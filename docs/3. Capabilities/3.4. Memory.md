...
!!! abstract "In one glance"
    ...
    - **You need:** [3.1. Tools](./3.1.%20Tools.md) finished; `cd agents/python && mise run config:check` passing if you want to run the retrieval evaluation.
...
The app plugin calls it from `before_model_callback`, meaning ADK runs it just before each model call. It keeps only the most recent messages and replaces the older span with a single synthetic note. It is off by default (unset sends the full history), deterministic, and model-free, so the offline test gate stays exact:
