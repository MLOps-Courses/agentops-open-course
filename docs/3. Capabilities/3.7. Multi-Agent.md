...
Neither specialist attaches policy callbacks. [`delegation.py`](https://github.com/MLOps-Courses/agentops-open-course/blob/main/agents/python/src/agent/delegation.py) defines only capabilities; `AgentOpsPolicyPlugin` governs every specialist through the enclosing `App`. An agent therefore cannot opt out of budget, redaction, or action policy by omitting callback wiring.
