...
You cannot, out of the box — and that is worth knowing before you go looking for a task. `coordinator_agent` is not the exported `root_agent`, there is no `mise` task for it, and ADK CLI discovery (`adk web src`, `adk run`) resolves only the two entrypoints it can load a `root_agent` from: the `agent` package and `agent.structured_report` (through its `agent` submodule, as [3.0. Packaging](./3.0.%20Packaging.md) shows). Today the coordinator's only exerciser is `tests/test_delegation.py`, which asserts the wiring structurally with no model call.
