...
The skills above are _runtime_ skills: `SKILL.md` files under `agents/data/skills` that this agent loads at execution time. The same format also travels the other direction — as guidance for the human or coding agent _building_ an agent. The repository ships a second, top-level [`skills/`](https://github.com/MLOps-Courses/agentops-open-course/tree/main/skills) directory of exactly that: portable Agent Skills that distil the course's operational patterns — telemetry, guardrails, resilience, token budgets, least privilege, evaluation, incident response — so you can apply them in your own projects rather than re-reading the course each time.
