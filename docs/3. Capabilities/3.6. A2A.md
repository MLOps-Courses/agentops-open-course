...
`create_app()` builds an explicit runner with `DatabaseSessionService` and an A2A `DatabaseTaskStore`, both backed by `.state/runtime.db`. Its lifespan closes the runner, session service, and SQLAlchemy engine. This is a single-process course durability model, not a horizontally scalable database design.
...
```bash
curl -N -fsS http://localhost:3001/ \
...
Verify card fields, advertised address, persistent service/task construction, shutdown cleanup, and the coordinator's sub-agent wiring. No model call is needed for this contract test.
