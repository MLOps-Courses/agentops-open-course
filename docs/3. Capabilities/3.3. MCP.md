...
`mcp_server.py` defaults to stdio for an agent that launches its own child process. It also supports SSE and streamable HTTP when explicitly configured:
...
Using `sys.executable` keeps the child in the same locked Python environment.
...
On the host with a local/forwarded gateway, use `http://localhost:3000/mcp`. The deployed call path is agent -> agentgateway -> `agentops-mcp:8000`; guarded writes and instruction-only skills remain local to the agent.
