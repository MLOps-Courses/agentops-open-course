...
1. The `presidio-analyzer`/`presidio-anonymizer` pair is pinned to the _same_ release (`2.2.362`) and held there deliberately: the next release (`2.2.363`) caps `cryptography` below the course's security floor, so both stay on `2.2.362` in lockstep.
...
The image copies `/app/data` in owned by the non-root uid; it is the deployment's `readOnlyRootFilesystem` (Chapter 6.1) that makes the whole root filesystem — `/app/data` included — read-only at runtime, while `/app/state` stays a writable volume. No machine-specific absolute path is committed; the defaults are computed, and deployments parse their own via the typed `Settings` boundary.
