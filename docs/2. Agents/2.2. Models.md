...
!!! tip "Try a different model — but measure, do not assume"
    ...
    - **`gemma4:e4b`** — Google's Gemma 4, also Apache-2.0 and ungated, with native function calling plus vision and audio. It is a genuinely strong model. It is **not** a like-for-like swap: despite the "E4B" name meaning ~4.5B *effective* parameters, it carries 8B parameters with embeddings and lands around **9.6 GB** (~6.1 GB for the `e4b-it-qat` build) — roughly four times the footprint of the default. That is the only reason it is not the default here, and if your machine has the memory it is well worth comparing.
