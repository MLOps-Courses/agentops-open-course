...
The distinction decides _who_ enforces a rule. A tool trusts the model to ask correctly. A policy hook does not ask the model to police itself: it can block, transform, or replace work before or after the model or a tool runs.
...
??? note "Deeper: why is the hook order load-bearing?"
...
Those functions live in `budget.py`, `compaction.py`, `pii.py`, and `guardrails.py`; the plugin composes them once. Per-agent callback lists made governance a clerical duty, so a newly added agent could silently omit redaction or budgeting. The `App` plugin now governs the root agent, every sub-agent, and every workflow node by construction. [2.1. First Agent](./2.1.%20First%20Agent.md#which-module-owns-each-tool-and-policy-hook) shows the boundary, and [4.5. Guardrails](../4.%20Quality/4.5.%20Guardrails.md#what-is-a-guardrail) owns the full pipeline.
...
| Policy plugin      | Policy at the model/tool boundaries | `governance.py` composes `budget.py`, `compaction.py`, `pii.py`, and `guardrails.py` ([4.5. Guardrails](../4.%20Quality/4.5.%20Guardrails.md#what-is-a-guardrail)) |
| Runner             | Drives one invocation               | `server.py` builds `Runner(app=build_app(...), …)`; the deprecated `agent=` form is not used ([2.4. Sessions](./2.4.%20Sessions.md#why-does-persistence-matter))   |
