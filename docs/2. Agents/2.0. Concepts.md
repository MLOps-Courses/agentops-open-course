...
Before you build the Ops Copilot, it helps to know the handful of pieces every Google ADK 2.0 agent is made of. This page is language-neutral: the same concepts hold in both the Python and Go tracks, with matching names. Later sections show the code.
...
You describe _what_ the agent is — its model, instructions, and tools — and ADK runs it. It ships as `google-adk` on PyPI (import `google.adk`) and as the Go module `google.golang.org/adk/v2`.

## What is an agent in ADK?

The core type is the **LlmAgent**: a language model wrapped with an identity (name and description), a system **instruction** (its persona and rules), and a set of **tools** it may call. In Python the public alias is simply `Agent`; in Go you build one with `llmagent.New`.
...
The Runner is also where the agent's services plug in: the session store, the artifact store, and long-term memory. For local development, ADK provides in-memory versions of all three, so a first agent needs no database — covered in [2.5. Sessions](./2.5. Sessions.md).
...
The rest of this chapter builds that stack end to end: the full agent in [2.1](./2.1. First Agent (Python).md) / [2.2](./2.2. First Agent (Go).md), then the model, instructions, sessions, and dev loop one page at a time.
