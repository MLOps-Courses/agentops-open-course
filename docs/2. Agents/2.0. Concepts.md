...
An `Agent` declaration is configuration, not a network call. A runner and a message are needed before inference begins.
...
The Ops Copilot registers request/response/tool-output PII callbacks, a before-tool action validator, and model/tool error callbacks. A callback can block, transform, or replace work without asking the model to police itself.
...
Open `agents/python/src/agent/agent.py` and identify the model, instruction, tools, and seven callbacks. Then open `server.py` and identify which resources are created once per application and closed during lifespan shutdown. If you cannot explain who owns sessions and tools, pause before adding capabilities.
